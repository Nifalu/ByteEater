...
const AUTH_URL: &str = "https://identitytoolkit.googleapis.com/v1/accounts:signInWithCustomToken";
// Public Firebase Web API key — not a secret, just identifies the project
const FIREBASE_API_KEY: &str = "AIzaSyDR_h6fX9hBl2I7oy7xjzmYgDxJY2igdls"; // gitleaks:allow
...
/// Fetch and decode the menu for a given ISO week.
...
/// Get a guest identity from qnips, then exchange it for a Firebase ID token.
fn authenticate() -> Result<String> {
    ...
    let auth_url = format!("{AUTH_URL}?key={FIREBASE_API_KEY}");
    let auth = http_post(
        &auth_url,
...
/// POST request via curl, returns parsed JSON.
...
/// Convert Firestore's typed wrapper format ({stringValue: "x"}, {mapValue: {fields: ...}}, etc.) into plain JSON.
fn unfire(v: &Value) -> Value {
    ...
    if let Some(mv) = m.get("mapValue").and_then(|v| v.as_object()) {
        return match mv.get("fields").and_then(|v| v.as_object()) {
            Some(fields) => {
                Value::Object(fields.iter().map(|(k, v)| (k.clone(), unfire(v))).collect())
            }
